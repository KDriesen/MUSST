...
use data_arch,       only : I4, I8, R8, EPS_R8, HIG_R8, OPU
...
public :: MAT_SOLV, MS_MAT_SOLV, solve_syst, MUMP, UMFP, SULU, SOLVER_BS, SOLVER_TS, SOLV_MESS, convert_matrice_format
   ...
   type(MAT_SOLV), intent(inout) :: mat   !! *high level system type*
      ...
      select case (mat%slv_t)
         ...
         case(MUMP)
            ...
            mat%matmump%icntl(1) = output_unit  ! output stream for error messages
            mat%matmump%icntl(2) = error_unit   ! output stream for diagnostic printing, statistics, and warning messages.
            mat%matmump%icntl(3) = error_unit   ! output stream for global information, collected on the host.
            mat%matmump%icntl(4) = 1            ! level of printing for error, warning, and diagnostic messages. 1 : only errors
